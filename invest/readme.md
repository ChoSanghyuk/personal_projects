...
추가적으로, 종목 가격 크롤링에 사용할 path 정보를 optional하게 관리합니다. (통화가 USD일 경우에만 필요)
...
- [ ] 종목 정보 저장
  - [ ] 종목 정보 저장/갱신 API (입력 : 이름, 구분, 통화, 기준 매도가, 기준 매수가 )
    - 입력으로 온 구분 값이 1~5의 정수인지 검증
    - 고점, 저점은 자체 크롤링으로 정보 조회하여 저장
    - 기준 매수가 미존재 시, 저점값 사용
  - [ ] 종목 정보 개별 조회
  - [ ] 종목 리스트 조회 (통화, 투자된 총액 같이 조회)
  - [ ] 종목 정보 삭제
...
- [ ] 보유하고 있는 종목에 대한 현재가 크롤링
  - [ ] :heavy_exclamation_mark: 종목 정보를 가지고 동적으로 현재가 정보를 가져올 수 있는 기능 필요 (가능한 사이트 사전 탐색)
    - 한국 종목 => 네이버 조회
    - 외국 종목 => investing.com 조회. 이때, 종목 별 url 뒷 부분 정보 필요함
...
![image-20240827000122132](img/image-20240827000122132.png)
